...
describe("metin içerikleri", () => {
  ...
  test("[3] ana içerik başlığı doğru kelimeyi içeriyor", () => {
    expect(
      screen.getByText("Özellikler", {
        selector: ".top-content .text-content:nth-of-type(1) h4",
      })
    );
    expect(
      screen.getByText("Hakkında", {
  ...
  test("[4] ana içerik paragrafları doğru metinleri içeriyor", () => {
    expect(
      screen.getByText(/Özellikler içeriği elementum magna eros/, {
        selector: ".top-content .text-content p",
      })
    );
    expect(
      screen.getByText(/Hakkında içeriği elementum magna eros/, {
        selector: ".top-content .text-content p",
      })
    );
  });
  test("[5] aşağıdaki başlıklar doğru kelimeleri içeriyor", () => {
    expect(
      screen.getByText("Servisler", {
        selector: ".bottom-content .text-content h4",
      })
    );
    expect(
      screen.getByText("Ürünler", {
        selector: ".bottom-content .text-content h4",
      })
    );
    expect(
      screen.getByText("Vizyon", {
        selector: ".bottom-content .text-content h4",
      })
    );
  });
  test("[6] bottom content headings contain the correct text", () => {
    expect(
      screen.getByText(/Servisler içeriği elementum magna eros/, {
        selector: ".bottom-content .text-content p",
      })
    );
    expect(
      screen.getByText(/Ürünler içeriği elementum magna eros/, {
        selector: ".bottom-content .text-content p",
      })
    );
    expect(
      screen.getByText(/Vizyon içeriği elementum magna eros/, {
        selector: ".bottom-content .text-content p",
      })
    );
  });
  test("[7] iletişim bilgileri doğru kelimeleri içeriyor", () => {
    expect(screen.getByText("İletişim", { selector: "section.contact h4" }));
    expect(
      screen.getByText(
    ...
    expect(
      screen.getByText("+90 (123) 456-7899", { selector: "section.contact p" })
    );
    expect(
      screen.getByText("satis@birsirketsitesi.com.tr", {
        selector: "section.contact p",
      })
    );
  });
  test("[8] copyright bilgisi doğru kelimeleri içeriyor", () => {
    expect(
      screen.getByText(/Copyright Bir Şirket Sitesi 2022/, {
        selector: "footer a",
      })
    );
  });
...
describe("img sources", () => {
  test("[9] üstteki logo img src si doğru", () => {
    expect(screen.getByAltText(/Bir Şirket Sitesi logo/)).toHaveAttribute(
      "src",
      "http://localhost:9000/img/logo.png"
    );
  });
  test("[10] cta img src si doğru", () => {
    expect(screen.getByAltText(/Bir kod parçası resmi/)).toHaveAttribute(
      "src",
  ...
  test("[11] ortadaki img doğru src ye sahip", () => {
    expect(
      screen.getByAltText(/Ekran boyunca bir kod parçası resmi/)
